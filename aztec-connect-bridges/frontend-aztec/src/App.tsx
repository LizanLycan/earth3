...
const App = () => {
  ...
  async function initUsersAndPrintBalances() {
    ...
    const userExists = await sdk!.userExists(accountPublicKey!)
    console.log('userExists', userExists)

    let account0 = userExists
      ? await sdk!.getUser(accountPublicKey!)
      : await sdk!.addUser(accountPrivateKey!);

    setAccount0(account0!);

    const stETHPTAsset = await sdk?.getAssetIdByAddress(EthAddress.fromString('0x137189d1342aabe7cd75b42b265e4647596aaa01')) || 0;
    
    console.log('get stETHPT info', await sdk?.getAssetInfo(stETHPTAsset))

    console.log('account0', account0)
  
    const isAccountRegistered = await sdk?.isAccountRegistered(accountPublicKey!)
    if (isAccountRegistered)
      setUserExists(true);

    console.log('isAccountRegistered', isAccountRegistered)
    ...
    console.log(
      "stETHPT balance",
      sdk!.fromBaseUnits(
        await sdk!.getBalance(account0.id, stETHPTAsset)
      )
    );
  }
  ...
  async function bridgeCrvLido() {
    ...
    if (account0 && spendingSigner && sdk) {
      let txId = await aztecConnect(
        account0,
        spendingSigner,
        0, // Testnet CurveStEthBridge
        fromAmount,
        "ETH",
        "WSTETH",
        undefined,
        undefined,
        1000000000000000, // Min acceptable amount of stETH per ETH 0.001
        TxSettlementTime.INSTANT,
        sdk
      )
  ...
  async function depositBridgeApWine() {
    const fromAmount: bigint = ethers.utils
      .parseEther(amount.toString())
      .toBigInt();
    // TODO: Catch error when fromAmount > user's available amount.
  ...
  async function withdrawBridgeApWine() {
    const fromAmount: bigint = ethers.utils
      .parseEther(amount.toString())
      .toBigInt();
    // TODO: Catch error when fromAmount > user's available amount.
          ...
          <div>
            ...
            {spendingSigner && account0 ? (
              <div>
                ...
                <button onClick={() => depositBridgeApWine()}>Deposit to Lido-ApWine strategy</button>
                <button onClick={() => withdrawBridgeApWine()}>Withdraw form Lido-ApWine strategy</button>
              </div>
            ) : (
              ""
            )}
